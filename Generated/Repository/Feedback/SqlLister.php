...
use Kodbazis\Generated\Feedback\Listing\CountedFeedbacks;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedFeedbacks
    {
        try {
            ...
            return new CountedFeedbacks(
                array_map(function ($array) {
                    return new Feedback((int)$array['id'], (string)$array['name'], (string)$array['content'], (int)$array['episodeId'], (int)$array['createdAt']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `feedbacks` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
