...
use Kodbazis\Generated\Post\Listing\CountedPosts;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedPosts
    {
        try {
            ...
            return new CountedPosts(
                array_map(function ($array) {
                    return new Post((int)$array['id'], (string)$array['title'], (string)$array['slug'], (string)$array['imgUrl'], (int)$array['createdAt'], (int)$array['publishedAt'], (string)$array['description'], (string)$array['content'], (string)$array['imgAltTag'], (bool)$array['isActive']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `posts` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
