...
use Kodbazis\Generated\Course\Listing\CountedCourses;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedCourses
    {
        try {
            ...
            return new CountedCourses(
                array_map(function ($array) {
                    return new Course((int)$array['id'], (string)$array['title'], (string)$array['content'], (string)$array['slug'], (string)$array['imgUrl'], (string)$array['videoId'], (string)$array['description'], (int)$array['createdAt'], (bool)$array['isActive'], (int)$array['price'], (int)$array['discount']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `courses` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
