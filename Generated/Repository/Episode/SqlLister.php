...
use Kodbazis\Generated\Episode\Listing\CountedEpisodes;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedEpisodes
    {
        try {
            ...
            return new CountedEpisodes(
                array_map(function ($array) {
                    return new Episode((int)$array['id'], (string)$array['title'], (string)$array['slug'], (int)$array['courseId'], (string)$array['imgUrl'], (string)$array['description'], (string)$array['content'], (int)$array['createdAt'], (int)$array['position'], (bool)$array['isActive']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `episodes` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
