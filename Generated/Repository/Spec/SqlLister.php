...
use Kodbazis\Generated\Spec\Listing\CountedSpecs;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedSpecs
    {
        try {
            ...
            return new CountedSpecs(
                array_map(function ($array) {
                    return new Spec((int)$array['id'], (string)$array['content'], (int)$array['position'], (int)$array['courseId'], (int)$array['createdAt']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `specs` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
