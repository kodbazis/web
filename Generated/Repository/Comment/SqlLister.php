...
use Kodbazis\Generated\Comment\Listing\CountedComments;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedComments
    {
        try {
            ...
            return new CountedComments(
                array_map(function ($array) {
                    return new Comment((int)$array['id'], (string)$array['content'], (int)$array['embeddableId'], (string)$array['key'], (int)$array['createdAt']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `comments` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
