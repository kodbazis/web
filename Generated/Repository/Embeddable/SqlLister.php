...
use Kodbazis\Generated\Embeddable\Listing\CountedEmbeddables;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedEmbeddables
    {
        try {
            ...
            if (!$stmt) { 
                return new CountedEmbeddables([], 0);
            }
            ...
            return new CountedEmbeddables(
                array_map(function ($array) {
                    return new Embeddable((int)$array['id'], (int)$array['createdAt'], (string)$array['name'], (string)$array['raw'], (string)$array['type']);
                }, $results),
                $count
            );
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `embeddables` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
    ...
    private function getCountQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `embeddables` %s %s %s",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
                $query->getOrderBy()->getValue()
