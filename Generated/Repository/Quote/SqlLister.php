...
use Kodbazis\Generated\Quote\Listing\CountedQuotes;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedQuotes
    {
        try {
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `quotes` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
