...
use Kodbazis\Generated\Coupon\Listing\CountedCoupons;
...
class SqlLister implements Lister
{
    ...
    public function list(Query $query): CountedCoupons
    {
        try {
    ...
    private function getSelectQuery(Query $query): callable
    {
        return function ($cols) use ($query) {
            ...
            return sprintf(
                "SELECT %s FROM `coupons` %s %s %s LIMIT ? OFFSET ?",
                $cols,
                $vals === '' ? '' : 'WHERE',
                $vals,
